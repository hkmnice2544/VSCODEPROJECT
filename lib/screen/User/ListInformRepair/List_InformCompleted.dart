...
class _MyWidgetState extends State<InformCompleted> {
  ...
  Future<String> ifreview(int informrepair_id) async {
    String result = await reviewController.countReview(informrepair_id);
    return result;
  }

  Future<void> processReportRepair() async {
    for (int i = 0; i < reportRepair!.length; i++) {
      int id = reportRepair?[i].informrepair?.informrepair_id ?? 0;
      String results = await ifreview(id); // Add await here
      if (results == "0") {
        isReview.add(true);
      } else {
        isReview.add(false);
      }
      print(reportRepair?[i].informrepair?.informrepair_id);
      print(isReview[i]);
    }
    setState(() {
      isDataLoaded = true;
